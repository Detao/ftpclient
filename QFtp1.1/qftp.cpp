...
void QFtpDTP::timerEvent(QTimerEvent *event)
{
    socketError(QTcpSocket::SocketTimeoutError);
}
...
void QFtpDTP::socketError(QAbstractSocket::SocketError e)
{
    if (e == QTcpSocket::HostNotFoundError) {
        ...
        emit connectState(QFtpDTP::CsConnectionTimeout);
    }
}
...
void QFtpDTP::socketConnectionClosed()
{
    ...
    // bytesFromSocket = socket->readAll(); //error close printf device not open
#if defined(QFTPDTP_DEBUG)
    qDebug("QFtpDTP::connectState(CsClosed)");
#endif
    emit connectState(QFtpDTP::CsClosed);
...
void QFtpPI::timerEvent( QTimerEvent *event )
{
    emit error(QFtp::NotConnected, QFtp::tr("Time out"));
}
...
bool QFtpPI::processReply()
{
    ...
    }else if(replyCodeInt==550){
        if(currentCmd.startsWith(QLatin1String("CWD ")))
        {
            pendingCommands.push_front(currentCmd);
            std::string dir=currentCmd.toStdString();
            dir=dir.substr(4);
            QString cmd=QLatin1String("MKD ") + QString::fromStdString(dir);
...
void QFtpPI::dtpConnectState(int s)
{
    ...
    case QFtpDTP::CsConnectionRefused:
        ...
        startNextCmd();
        return;
    case QFtpDTP::CsConnectionTimeout:
        emit error(QFtp::ConnectionRefused,
                   QFtp::tr("Connection timeout for data connection"));
        startNextCmd();
        return;
    default:
        return;
...
int QFtp::put(QIODevice *dev, const QString &file, TransferType type)
{
    ...
    std::size_t num;
    while((num=filepath.find_first_of("/"))!=std::string::npos)
    {
        std::string path=filepath.substr(0,num);
        if(!path.empty())
            cmds<<QLatin1String("CWD ")+QString::fromStdString(path)+QLatin1String("\r\n");
        filepath=filepath.substr(num+1);
    }
    QString remotefile=QString::fromStdString(filepath);
    if (!dev->isSequential())
        cmds << QLatin1String("ALLO ") + QString::number(dev->size()) + QLatin1String("\r\n");
    cmds << QLatin1String("STOR ") + remotefile + QLatin1String("\r\n");
    //qDebug() << cmds;
...
int QFtp::conPut(QIODevice *dev, const QString &file, TransferType type)
{
    ...
    if (!dev->isSequential())
        cmds << QLatin1String("ALLO ") + QString::number(dev->size()) + QLatin1String("\r\n");
    cmds<<  QLatin1String("APPE ") + remotefile + QLatin1String("\r\n");
